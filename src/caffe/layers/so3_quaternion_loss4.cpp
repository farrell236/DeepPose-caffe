...
void SO3QuaternionLoss4Layer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void SO3QuaternionLoss4Layer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                             const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void SO3QuaternionLoss4Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                                 const vector<Blob<Dtype>*>& top) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        loss[0] += Dtype(1) - fabs(dot_data[i]);
...
template <typename Dtype>
void SO3QuaternionLoss4Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    for (int i = 0; i < N; i++) {
        ...
        Dtype coeff = Dtype(-1) * ((dot_data[i] > 0)?1:-1) ;
...
#ifdef CPU_ONLY
STUB_GPU(SO3QuaternionLoss4Layer);
#endif

INSTANTIATE_CLASS(SO3QuaternionLoss4Layer);
REGISTER_LAYER_CLASS(SO3QuaternionLoss4);
